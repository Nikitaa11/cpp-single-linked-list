...
class SingleLinkedList {
    ...
    class BasicIterator {
        ...
        explicit BasicIterator(const Node* node) {
...
private:
    // Шаблонный метод для инициализации из диапазона итераторов
    template <typename InputIterator>
    void InitializeFromRange(InputIterator begin, InputIterator end) {
        ...
        while (!temp.IsEmpty()) {
            PushFront(temp.head_.next_node->value);
            Node* next = temp.head_.next_node->next_node;
            delete temp.head_.next_node;
            temp.head_.next_node = next;
            --temp.size_;
        }
    }
